...
os.environ.setdefault("JWT_SECRET_KEY", "test-secret-key-for-development-32-chars-minimum-length")
...
def test_password_security():
    ...
    try:
        from app.auth.security import hash_password, verify_password, is_password_strong
        ...
        assert len(errors_strong) == 0, "Strong password should have no validation errors"
        ...
        assert hashed != test_password, "Hashed password should not equal plain password"
        assert verify_password(test_password, hashed), "Password verification should succeed"
...
def test_jwt_tokens():
    ...
    try:
        from app.auth.security import create_access_token, create_refresh_token, decode_token, verify_token
        ...
        assert refresh_token != access_token, "Refresh token should differ from access token"
        
        print("✓ Refresh token creation working")
        
        # Test token decoding
        decoded_payload = decode_token(access_token)
        assert decoded_payload["sub"] == user_data["sub"], "Decoded subject should match"
        assert decoded_payload["email"] == user_data["email"], "Decoded email should match"
        ...
        assert verified_payload["sub"] == user_data["sub"], "Verified payload should match"
...
def test_schemas():
    ...
    try:
        from app.auth.schemas import UserCreate, UserLogin, Token, is_password_strong
        ...
        assert not is_strong_weak and len(errors_weak) > 0, "Weak password validation failed"
        assert is_strong_strong and len(errors_strong) == 0, "Strong password validation failed"
        ...
        try:
            ...
            assert "Password requirements not met" in str(e), "Should get password validation error"
