...
from tests.factories import UserFactory, VerifiedUserFactory, SSHProfileFactory, SSHKeyFactory
...
class TestSSHProfileEndpoints:
    ...
    async def test_get_ssh_profiles_success(self, async_client, auth_headers, test_user, test_session):
    ...
    async def test_get_ssh_profile_by_id_success(self, async_client, auth_headers, test_user, test_session):
        """Test getting specific SSH profile by ID."""
        profile = SSHProfileFactory(user_id=test_user.id)
        test_session.add(profile)
        await test_session.commit()
        
        response = await async_client.get(f"/api/ssh/profiles/{profile.id}", headers=auth_headers)
    ...
    async def test_get_ssh_profile_not_found(self, async_client, auth_headers):
        ...
        response = await async_client.get(f"/api/ssh/profiles/{fake_id}", headers=auth_headers)
        
        assert response.status_code == status.HTTP_404_NOT_FOUND
    
    async def test_get_ssh_profile_unauthorized_access(self, async_client, auth_headers, test_session):
    ...
    async def test_update_ssh_profile_success(self, async_client, auth_headers, test_user, test_session):
    ...
    async def test_delete_ssh_profile_success(self, async_client, auth_headers, test_user, test_session):
        """Test successful SSH profile deletion."""
        profile = SSHProfileFactory(user_id=test_user.id)
        test_session.add(profile)
        await test_session.commit()
        
        response = await async_client.delete(f"/api/ssh/profiles/{profile.id}", headers=auth_headers)
    ...
    async def test_test_ssh_connection_success(self, mock_ssh_client, async_client, auth_headers, test_user, test_session):
        ...
        response = await async_client.post(
            f"/api/ssh/profiles/{profile.id}/test",
            headers=auth_headers
        )
    ...
    @patch('app.api.ssh.service.SSHClient')
    async def test_test_ssh_connection_failure(self, mock_ssh_client, async_client, auth_headers, test_user, test_session):
...
class TestSSHKeyEndpoints:
    ...
    async def test_get_ssh_keys_success(self, async_client, auth_headers, test_user, test_session):
    ...
    async def test_get_ssh_key_by_id_success(self, async_client, auth_headers, test_user, test_session):
        """Test getting specific SSH key by ID."""
        key = SSHKeyFactory(user_id=test_user.id)
        test_session.add(key)
        await test_session.commit()
        
        response = await async_client.get(f"/api/ssh/keys/{key.id}", headers=auth_headers)
    ...
    async def test_update_ssh_key_success(self, async_client, auth_headers, test_user, test_session):
    ...
    async def test_delete_ssh_key_success(self, async_client, auth_headers, test_user, test_session):
        """Test successful SSH key deletion."""
        key = SSHKeyFactory(user_id=test_user.id)
        test_session.add(key)
        await test_session.commit()
        
        response = await async_client.delete(f"/api/ssh/keys/{key.id}", headers=auth_headers)
    ...
    async def test_ssh_key_usage_tracking(self, async_client, auth_headers, test_user, test_session):
...
class TestSSHProfileKeyAssociation:
    ...
    async def test_assign_key_to_profile_success(self, async_client, auth_headers, test_user, test_session):
    ...
    async def test_assign_nonexistent_key_to_profile(self, async_client, auth_headers, test_user, test_session):
    ...
    async def test_assign_other_users_key_to_profile(self, async_client, auth_headers, test_user, test_session):
    ...
    async def test_remove_key_from_profile_success(self, async_client, auth_headers, test_user, test_session):
...
class TestSSHConfigGeneration:
    ...
    async def test_generate_ssh_config_single_profile(self, async_client, auth_headers, test_user, test_session):
        ...
        response = await async_client.get(
            f"/api/ssh/profiles/{profile.id}/config",
            headers=auth_headers
        )
    ...
    async def test_generate_ssh_config_all_profiles(self, async_client, auth_headers, test_user, test_session):
        """Test generating SSH config for all user profiles."""
        # Create multiple profiles
        key1 = SSHKeyFactory(user_id=test_user.id)
        key2 = SSHKeyFactory(user_id=test_user.id)
        
        profile1 = SSHProfileFactory(user_id=test_user.id, ssh_key_id=key1.id, name="server1")
        profile2 = SSHProfileFactory(user_id=test_user.id, ssh_key_id=key2.id, name="server2")
...
class TestSSHStatistics:
    ...
    async def test_get_ssh_profile_statistics(self, async_client, auth_headers, test_user, test_session):
        ...
        response = await async_client.get("/api/ssh/profiles/statistics", headers=auth_headers)
    ...
    async def test_get_ssh_key_statistics(self, async_client, auth_headers, test_user, test_session):
        ...
        response = await async_client.get("/api/ssh/keys/statistics", headers=auth_headers)
