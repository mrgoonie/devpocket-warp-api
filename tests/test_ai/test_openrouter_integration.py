...
class TestAIEndpoints:
    ...
    async def test_ai_endpoint_invalid_api_key(self, test_client, auth_headers):
        ...
        with patch('app.api.ai.service.AIService.validate_user_api_key') as mock_validate:
...
class TestBYOKModel:
    ...
    async def test_byok_usage_tracking(self, ai_service):
        ...
        with patch.object(ai_service, 'openrouter_service') as mock_service:
            ...
            # Act
            await ai_service.suggest_command(CommandSuggestionRequest(
                prompt="list files",
                api_key=api_key
            ))
    ...
    async def test_byok_cost_calculation(self, ai_service):
        ...
        with patch.object(ai_service, 'openrouter_service') as mock_service:
            ...
            # Act
            result = await ai_service.suggest_command(CommandSuggestionRequest(
                prompt="test",
                api_key="test-key"
            ))
