...
class TestRedisErrorHandling:
    ...
    async def test_redis_memory_exhaustion(self, test_client, auth_headers):
        ...
        # Arrange
        with patch('app.websocket.manager.connection_manager.redis_client') as mock_redis:
            mock_redis.set.side_effect = redis.exceptions.ResponseError("OOM command not allowed")
    ...
    async def test_redis_timeout(self, test_client):
        ...
        # Arrange
        with patch('app.auth.security.redis_client') as mock_redis:
            mock_redis.get.side_effect = redis.exceptions.TimeoutError("Operation timed out")
...
class TestGracefulDegradation:
    ...
    async def test_ai_service_degradation(self, test_client, auth_headers):
        ...
        # Arrange
        with patch('app.services.openrouter.OpenRouterService.generate_command') as mock_ai:
    ...
    async def test_sync_service_degradation(self, test_client, auth_headers):
        ...
        with patch('app.api.sync.service.SyncService.sync_data') as mock_sync:
            ...
            response = test_client.post(
                "/api/sync/data",
                json={"sync_type": "command_history", "data": {"command": "ls"}},
                headers=auth_headers
            )
