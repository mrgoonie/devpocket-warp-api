...
class TestDbSeedScript:
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_clean_option_with_confirmation(self, mock_file, mock_run, script_runner, mock_env):
    ...
    def test_clean_force_option(self, mock_file, mock_run, script_runner, mock_env):
        ...
        with patch.dict(os.environ, mock_env):
            result = script_runner.run_script("db_seed.sh", ["--clean-force", "users", "10"])
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_clean_specific_data_types(self, mock_file, mock_run, script_runner, mock_env):
        ...
        with patch.dict(os.environ, mock_env):
            result = script_runner.run_script("db_seed.sh", ["--clean-force", "ssh", "5"])
    ...
    def test_reset_force_option(self, mock_file, mock_run, script_runner, mock_env):
        ...
        with patch.dict(os.environ, mock_env):
            result = script_runner.run_script("db_seed.sh", ["--reset-force", "all", "3"])
    ...
    def test_upsert_option(self, mock_file, mock_run, script_runner, mock_env):
        ...
        assert "on_conflict_do_nothing" in written_content or "upsert" in written_content.lower()
    ...
    def test_env_file_option(self, mock_file, mock_run, script_runner, temp_dir):
        ...
        result = script_runner.run_script(
            "db_seed.sh", 
            ["--env-file", str(custom_env_file), "--stats-only"]
        )
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_clean_cancellation_by_user(self, mock_file, mock_run, script_runner, mock_env):
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_reset_cancellation_by_user(self, mock_file, mock_run, script_runner, mock_env):
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_cleaning_script_content_all_types(self, mock_file, mock_run, script_runner, mock_env):
        ...
        with patch.dict(os.environ, mock_env):
            result = script_runner.run_script("db_seed.sh", ["--clean-force", "all", "5"])
        
        assert result.returncode == 0
        
        # Check cleaning script content
        written_content = "".join(call.args[0] for call in mock_file().write.call_args_list)
        expected_tables = ["commands", "sessions", "ssh_profiles", "sync_data", "users"]
        for table in expected_tables:
            assert table in written_content.lower()
    
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_enhanced_seeding_with_randomization(self, mock_file, mock_run, script_runner, mock_env):
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_enhanced_ssh_profiles_with_relationships(self, mock_file, mock_run, script_runner, mock_env):
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_enhanced_commands_with_sessions(self, mock_file, mock_run, script_runner, mock_env):
        ...
        assert "ai_suggested" in written_content or "was_ai_suggested" in written_content
    
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_enhanced_sync_data_creation(self, mock_file, mock_run, script_runner, mock_env):
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_transaction_management_in_seeding(self, mock_file, mock_run, script_runner, mock_env):
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_error_handling_in_cleaning_script(self, mock_file, mock_run, script_runner, mock_env):
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_error_handling_in_reset_operation(self, mock_file, mock_run, script_runner, mock_env):
        ...
        with patch.dict(os.environ, mock_env):
            result = script_runner.run_script("db_seed.sh", ["--reset-force", "all", "5"])
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_comprehensive_logging_output(self, mock_file, mock_run, script_runner, mock_env):
        ...
        with patch.dict(os.environ, mock_env):
            result = script_runner.run_script("db_seed.sh", ["--upsert", "all", "5", "--stats"])
    ...
    @patch('subprocess.run')
    @patch('builtins.open', new_callable=mock_open)
    def test_foreign_key_constraint_handling(self, mock_file, mock_run, script_runner, mock_env):
...
class TestDbSeedIntegration:
    ...
    def test_stats_real(self, script_runner):
        ...
        assert result.returncode == 0
        output = result.stdout + result.stderr
        assert "Database Table Statistics" in output or "table statistics" in output.lower()
