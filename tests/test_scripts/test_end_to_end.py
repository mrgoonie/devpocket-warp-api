...
class TestEndToEndWorkflows:
    ...
    def run_script_with_timeout(self, script_runner, script_name: str, args: List[str], 
                               env: Dict[str, str], timeout: int = 60) -> subprocess.CompletedProcess:
    ...
    async def get_table_counts(self, db_connection) -> Dict[str, int]:
        ...
        tables = ['users', 'ssh_profiles', 'ssh_keys', 'sessions', 'commands', 'sync_data']
        counts = {}
    ...
    async def test_full_development_setup(self, script_runner, db_connection, db_env):
        ...
        assert counts['users'] >= 10, f"Expected at least 10 users, got {counts['users']}"
        assert counts['ssh_profiles'] >= 5, f"Expected SSH profiles, got {counts['ssh_profiles']}"
    ...
    @pytest.mark.slow
    async def test_production_deployment_simulation(self, script_runner, db_connection, db_env):
    ...
    async def test_data_migration_workflow(self, script_runner, db_connection, db_env):
        ...
        result = self.run_script_with_timeout(
            script_runner, "db_seed.sh", ["--clean-force", "commands", "0"], db_env
        )
    ...
    @pytest.mark.slow
    async def test_disaster_recovery_simulation(self, script_runner, db_connection, db_env):
        ...
        counts = await self.get_table_counts(db_connection)
        assert all(count == 0 for count in counts.values()), "All tables should be empty after disaster"
    ...
    @pytest.mark.slow
    async def test_performance_stress_workflow(self, script_runner, db_connection, db_env):
    ...
    @pytest.mark.slow
    async def test_complex_scenario_workflow(self, script_runner, db_connection, db_env):
        ...
        result = self.run_script_with_timeout(
            script_runner, "db_seed.sh", ["--clean-force", "users", "20"], db_env
        )
        assert result.returncode == 0
    ...
    @pytest.mark.slow
    async def test_incremental_update_workflow(self, script_runner, db_connection, db_env):
    ...
    def test_error_handling_workflow(self, script_runner, db_env):
        ...
        result = self.run_script_with_timeout(
            script_runner, "db_seed.sh", ["--stats-only"], invalid_env, timeout=30
        )
    ...
    @pytest.mark.slow
    async def test_concurrent_operation_simulation(self, script_runner, db_connection, db_env):
        ...
        result = self.run_script_with_timeout(
            script_runner, "db_seed.sh", ["--clean-force", "users", "10"], db_env
        )
        assert result.returncode == 0
...
class TestWorkflowReliability:
    ...
    def test_environment_variable_handling(self, script_runner, temp_dir):
        ...
        result = script_runner.run_script(
            "db_seed.sh", 
            ["--env-file", str(env_file), "--stats-only"]
        )
