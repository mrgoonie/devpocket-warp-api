...
class SSHProfileFactory(factory.Factory):
    ...
    ssh_options = factory.LazyFunction(lambda: json.dumps({
    ...
    last_used_at = factory.LazyFunction(lambda: datetime.now(timezone.utc) - timedelta(days=1))
...
class SSHKeyFactory(factory.Factory):
    ...
    key_size = factory.LazyAttribute(lambda obj: {
    ...
    # Mock public key
    public_key = factory.LazyAttribute(lambda obj: 
        f"ssh-{obj.key_type} {fake.lexify('?' * 64)} {fake.user_name()}@{fake.domain_name()}"
    )
    
    # Key metadata
    comment = factory.LazyAttribute(lambda obj: f"{fake.user_name()}@{fake.domain_name()}")
    has_passphrase = fuzzy.FuzzyChoice([True, False])
    
    # File system reference
    file_path = factory.LazyAttribute(lambda obj: 
        f"/home/{fake.user_name()}/.ssh/id_{obj.key_type}"
    )
    ...
    # Usage tracking
    last_used_at = factory.LazyFunction(lambda: datetime.now(timezone.utc) - timedelta(hours=6))
...
class RSAKeyFactory(SSHKeyFactory):
    ...
    public_key = factory.LazyFunction(lambda: 
        f"ssh-rsa {fake.lexify('?' * 64)} {fake.user_name()}@{fake.domain_name()}"
    )
...
class Ed25519KeyFactory(SSHKeyFactory):
    ...
    public_key = factory.LazyFunction(lambda: 
        f"ssh-ed25519 {fake.lexify('?' * 43)} {fake.user_name()}@{fake.domain_name()}"
    )
...
class ECDSAKeyFactory(SSHKeyFactory):
    ...
    public_key = factory.LazyFunction(lambda: 
        f"ssh-ecdsa {fake.lexify('?' * 64)} {fake.user_name()}@{fake.domain_name()}"
    )
...
class FrequentlyUsedSSHKeyFactory(SSHKeyFactory):
    ...
    last_used_at = factory.LazyFunction(lambda: datetime.now(timezone.utc) - timedelta(minutes=30))
