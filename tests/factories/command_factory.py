...
class CommandFactory(factory.Factory):
    ...
    # Command details
    command = factory.LazyFunction(lambda: fake.random_element([
        "ls -la",
        "cd /home/user",
        "git status",
        "npm install",
    ...
    # Command metadata
    working_directory = factory.LazyFunction(lambda: fake.random_element([
        "/home/user",
        "/var/www/html",
        "/opt/app",
        "/tmp",
    ...
    environment_vars = factory.LazyFunction(lambda: json.dumps({
...
class FailedCommandFactory(CommandFactory):
    ...
    status = "error"
    error_output = factory.LazyAttribute(lambda obj: _generate_error_output(obj.command))
    output = None
...
def _classify_command(command: str) -> str:
    ...
    if command_lower.startswith(('git ', 'gh ')):
        return "git"
    elif any(command_lower.startswith(cmd) for cmd in ['ls', 'cd', 'mkdir', 'rm', 'cp', 'mv', 'find']):
        return "file_operation"
    elif any(command_lower.startswith(cmd) for cmd in ['ping', 'curl', 'wget', 'ssh', 'scp']):
        return "network"
    elif any(command_lower.startswith(cmd) for cmd in ['ps', 'top', 'kill', 'systemctl', 'service']):
        return "system"
    elif any(command_lower.startswith(cmd) for cmd in ['docker', 'kubectl', 'python', 'node']):
        return "development"
    else:
        return "other"
