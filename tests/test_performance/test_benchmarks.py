...
class TestAPIPerformance:
    ...
    async def test_auth_endpoint_performance(self, performance_app, benchmark):
        ...
        async with AsyncClient(app=performance_app, base_url="http://test") as client:
            ...
            def auth_request():
                return asyncio.run(client.post("/api/auth/login", json={
            ...
            assert result.status_code in [200, 401]  # Should respond quickly regardless
            assert benchmark.stats["mean"] < 0.5  # Should complete within 500ms

    @pytest.mark.asyncio
    async def test_user_profile_performance(self, performance_app, auth_headers, benchmark):
        """Test user profile retrieval performance."""
        async with AsyncClient(app=performance_app, base_url="http://test") as client:
            
            def profile_request():
                return asyncio.run(client.get("/api/auth/profile", headers=auth_headers))
    ...
    @pytest.mark.asyncio
    async def test_ssh_profile_list_performance(self, performance_app, auth_headers, benchmark):
        """Test SSH profile listing performance."""
        async with AsyncClient(app=performance_app, base_url="http://test") as client:
            
            def ssh_profiles_request():
                return asyncio.run(client.get("/api/ssh/profiles", headers=auth_headers))
    ...
    @pytest.mark.asyncio
    async def test_concurrent_api_requests(self, performance_app, auth_headers, benchmark):
...
class TestDatabasePerformance:
    ...
    async def test_complex_query_performance(self, command_repository, benchmark):
        ...
        def complex_query():
            ...
            return asyncio.run(command_repository.get_user_command_statistics("user-123"))
    ...
    async def test_ai_command_suggestion_time(self, ai_service, benchmark):
        ...
        def ai_suggest():
            with patch.object(ai_service, 'openrouter_service') as mock_service:
                ...
                return asyncio.run(ai_service.suggest_command({
                    "prompt": "list files",
                    "api_key": "test-key"
                }))
...
class TestSynchronizationPerformance:
    ...
    async def test_real_time_sync_notification_time(self, sync_service, benchmark):
        ...
        def sync_notification():
            with patch.object(sync_service, 'redis_client') as mock_redis:
                return asyncio.run(sync_service.notify_sync_update(
                    "user-123", 
                    {"sync_type": "command_history", "data": {"command": "ls"}}
                ))
    ...
    async def test_conflict_resolution_time(self, sync_service, benchmark):
        ...
        def conflict_resolution():
            ...
            return sync_service.resolve_conflict(local_data, remote_data, "last_write_wins")
    ...
    async def test_database_load_testing(self, user_repository, benchmark):
        ...
        def database_load_test():
            async def concurrent_queries():
                tasks = [
                    user_repository.get_by_id(f"user-{i}")
                    for i in range(20)
                ]
