...
class CommandBase(BaseModel):
    ...
    command: str = Field(..., min_length=1, max_length=10000, description="Command text")
    working_directory: Optional[str] = Field(None, max_length=1000, description="Working directory")
    environment: Optional[Dict[str, str]] = Field(None, description="Environment variables")
    timeout_seconds: int = Field(default=30, ge=1, le=3600, description="Command timeout")
...
class CommandResponse(CommandBase):
    ...
    # Classification
    command_type: CommandType = Field(default=CommandType.UNKNOWN, description="Command type")
    is_dangerous: bool = Field(default=False, description="Potentially dangerous command flag")
    
    # Metadata
    pid: Optional[int] = Field(None, description="Process ID")
    signal: Optional[int] = Field(None, description="Signal that terminated the process")
    
    # History context
    sequence_number: int = Field(default=0, description="Sequence in session")
    parent_command_id: Optional[str] = Field(None, description="Parent command ID for pipes/chains")
...
class CommandSearchRequest(BaseModel):
    ...
    executed_before: Optional[datetime] = Field(None, description="Executed before date")
    ...
    working_directory: Optional[str] = Field(None, description="Filter by working directory")
    
    # Dangerous commands filter
    include_dangerous: bool = Field(default=True, description="Include dangerous commands")
...
class CommandUsageStats(BaseModel):
    ...
    # Top commands
    most_used_commands: List[Dict[str, Any]] = Field(..., description="Most frequently used commands")
    longest_running_commands: List[Dict[str, Any]] = Field(..., description="Longest running commands")
...
class CommandSuggestionRequest(BaseModel):
    ...
    context: str = Field(..., min_length=1, max_length=1000, description="Context or description")
    ...
    preferred_tools: Optional[List[str]] = Field(None, description="Preferred tools/utilities")
...
class CommandExportRequest(BaseModel):
    ...
    session_ids: Optional[List[str]] = Field(None, description="Specific sessions to export")
    ...
    max_commands: int = Field(default=10000, ge=1, le=50000, description="Maximum commands")
...
class BulkCommandOperation(BaseModel):
    ...
    command_ids: List[str] = Field(..., min_items=1, max_items=1000, description="Command IDs")
