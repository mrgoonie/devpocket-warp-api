...
from .schemas import (
    # Command schemas
    CommandResponse, CommandListResponse,
    CommandSearchRequest, CommandHistoryResponse,
    
    # Analytics schemas
    CommandUsageStats, SessionCommandStats, CommandTypeStats,
    FrequentCommandsResponse, CommandMetrics,
    
    # Suggestion schemas
    CommandSuggestion, CommandSuggestionRequest,
    
    # Export schemas
    CommandExportRequest, CommandExportResponse,
    
    # Batch operations
    BulkCommandOperation, BulkCommandResponse,
...
async def get_frequent_commands(
    ...
    service = CommandService(db)
    return await service.get_frequent_commands(current_user, days=days, min_usage=min_usage)
...
async def get_command_patterns(
    ...
    try:
        ...
        patterns = {
            ...
            "command_diversity": {
                "unique_vs_total": round(stats.unique_commands / max(stats.total_commands, 1), 3),
                "type_distribution": stats.commands_by_type
            },
            "efficiency_metrics": {
                "success_rate": round(stats.successful_commands / max(stats.total_commands, 1) * 100, 2),
    ...
    try:
        ...
        insights = {
            "execution_performance": {
                ...
                "total_execution_time_hours": round(stats.total_execution_time_ms / (1000 * 3600), 2),
                "slowest_commands": stats.longest_running_commands[:5]
            },
            "error_analysis": {
                "success_rate": round(stats.successful_commands / max(stats.total_commands, 1) * 100, 2),
                "failure_rate": round(stats.failed_commands / max(stats.total_commands, 1) * 100, 2),
                "error_distribution": metrics.top_error_types
            },
            "efficiency_recommendations": [],
            "resource_usage": {
                "commands_per_session_avg": round(stats.total_commands / max(len(set()), 1), 2),  # Would need session count
                "peak_usage_periods": {
        ...
        if stats.failed_commands / max(stats.total_commands, 1) > 0.1:  # 10% failure rate
            insights["efficiency_recommendations"].append({
                "type": "reliability",
    ...
    try:
        ...
        return {
            "summary": {
                ...
                "commands_today": stats.commands_today,
                "success_rate": round(stats.successful_commands / max(stats.total_commands, 1) * 100, 2),
                "most_used_type": max(stats.commands_by_type.items(), key=lambda x: x[1]) if stats.commands_by_type else ("unknown", 0)
            },
            "recent_activity": [
                {
            ...
            "top_commands": [
                {
                    "command": cmd["command"][:30] + "..." if len(cmd["command"]) > 30 else cmd["command"],
                    "count": cmd["count"]
                }
