...
from .schemas import (
    CommandResponse, CommandSearchRequest, CommandHistoryResponse, CommandHistoryEntry,
    CommandUsageStats, SessionCommandStats, CommandTypeStats,
    FrequentCommand, FrequentCommandsResponse,
    CommandSuggestion, CommandSuggestionRequest,
    CommandMetrics, CommandType, CommandStatus
)
...
class CommandService:
        ...
        try:
            ...
            for cmd in commands:
                entry = CommandHistoryEntry(
                    ...
                    output_size=len(cmd.stdout) + len(cmd.stderr) if cmd.stdout and cmd.stderr else 0,
    ...
    async def get_usage_stats(self, user: User) -> CommandUsageStats:
        ...
        try:
            ...
            successful_commands = len([cmd for cmd in all_commands if cmd.exit_code == 0])
            ...
            type_counter = Counter(cmd.command_type or "unknown" for cmd in all_commands)
            ...
            commands_this_week = len([
                cmd for cmd in all_commands
                if cmd.executed_at >= week_ago
            ])
            commands_this_month = len([
                cmd for cmd in all_commands
                if cmd.executed_at >= month_ago
            ])
        ...
        try:
            ...
            for pattern, data in command_analysis.items():
                if data["count"] >= min_usage:
                    ...
                    sessions_used = len(set(
                        cmd.session_id for cmd in commands 
    ...
    async def get_command_suggestions(
        ...
        try:
            ...
            if any(word in context_lower for word in ["list", "show", "files", "directory"]):
                suggestions.extend(self._get_file_operation_suggestions(context_lower))
            
            # System monitoring suggestions
            if any(word in context_lower for word in ["process", "memory", "cpu", "monitor"]):
                suggestions.extend(self._get_system_monitoring_suggestions(context_lower))
            
            # Network suggestions
            if any(word in context_lower for word in ["network", "connection", "ping", "download"]):
                suggestions.extend(self._get_network_suggestions(context_lower))
            
            # Git suggestions
            if any(word in context_lower for word in ["git", "repository", "commit", "branch"]):
                suggestions.extend(self._get_git_suggestions(context_lower))

            # Based on user's command history patterns
            if recent_commands:
                suggestions.extend(self._get_personalized_suggestions(recent_commands, context_lower))
    ...
    async def get_command_metrics(self, user: User) -> CommandMetrics:
        ...
        try:
            ...
            avg_response_time = (
                sum(cmd.duration_ms for cmd in completed_commands) / len(completed_commands)
                if completed_commands else 0
            )
    ...
    def _get_system_monitoring_suggestions(self, context: str) -> List[CommandSuggestion]:
