...
from .schemas import (
    # API Key schemas
    APIKeyValidationResponse, AIUsageStats,
    
    # Command suggestion schemas
    CommandSuggestionRequest, CommandSuggestionResponse, CommandSuggestion,
    
    # Command explanation schemas  
    CommandExplanationRequest, CommandExplanationResponse, CommandExplanation,
    
    # Error analysis schemas
    ErrorAnalysisRequest, ErrorAnalysisResponse, ErrorAnalysis,
    
    # Optimization schemas
    CommandOptimizationRequest, CommandOptimizationResponse, CommandOptimization,
    
    # Settings and models
    AISettings, AISettingsResponse, AIModelInfo, AvailableModelsResponse,
    
    # Batch processing
    BatchAIRequest, BatchAIResponse,
    
    # Enums
    AIServiceType, ConfidenceLevel
)
...
class AIService:
    ...
    async def suggest_command(
        ...
        try:
            # Check cache first
            cache_key = self._generate_cache_key("suggest", request.description, request.model)
    ...
    async def explain_command(
        ...
        try:
            # Check cache
            cache_key = self._generate_cache_key("explain", request.command, request.model)
            cached_response = self._get_cached_response(cache_key)
            if cached_response:
    ...
    async def analyze_error(
        ...
        try:
            # Check cache
            cache_key = self._generate_cache_key("error", f"{request.command}:{request.error_output}", request.model)
            cached_response = self._get_cached_response(cache_key)
            if cached_response:
            ...
            response = ErrorAnalysisResponse(
                ...
                error_summary=request.error_output[:200] + "..." if len(request.error_output) > 200 else request.error_output,
                model_used=ai_response.model,
                response_time_ms=ai_response.response_time_ms,
                tokens_used=ai_response.usage,
    ...
    async def optimize_command(
        self, 
        user: User, 
        request: CommandOptimizationRequest
    ) -> CommandOptimizationResponse:
        """Get command optimization suggestions using AI."""
        try:
            # Check cache
            cache_key = self._generate_cache_key("optimize", request.command, request.model)
            cached_response = self._get_cached_response(cache_key)
            if cached_response:
            ...
            for i, req_data in enumerate(request.requests):
                try:
                    ...
                    if request.service_type == AIServiceType.COMMAND_SUGGESTION:
                        result = await self._process_batch_suggestion(request.api_key, req_data)
                    elif request.service_type == AIServiceType.COMMAND_EXPLANATION:
                        result = await self._process_batch_explanation(request.api_key, req_data)
                    elif request.service_type == AIServiceType.ERROR_ANALYSIS:
                        result = await self._process_batch_error_analysis(request.api_key, req_data)
                    else:
    ...
    def _generate_cache_key(self, service_type: str, content: str, model: Optional[str]) -> str:
    ...
    def _parse_command_suggestions(
        ...
        try:
            ...
            if ai_response.content.strip().startswith('{') or ai_response.content.strip().startswith('['):
    ...
    def _parse_command_explanation(
        ...
        try:
            ...
            explanation = CommandExplanation(
                ...
                alternatives=self._extract_alternatives(content) if include_alternatives else []
            )
    ...
    def _calculate_confidence_score(self, suggestions: List[CommandSuggestion]) -> float:
    ...
    def _calculate_explanation_confidence(self, explanation: CommandExplanation) -> float:
    ...
    def _calculate_optimization_confidence(self, optimization: CommandOptimization) -> float:
