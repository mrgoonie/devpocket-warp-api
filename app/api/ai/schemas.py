...
class APIKeyValidation(BaseModel):
    ...
    api_key: str = Field(..., min_length=10, description="OpenRouter API key to validate")
...
class APIKeyValidationResponse(BaseModel):
    ...
    recommended_models: Optional[List[str]] = Field(None, description="Recommended models for DevPocket")
...
class CommandSuggestionRequest(BaseModel):
    ...
    description: str = Field(..., min_length=5, max_length=1000, description="Natural language description")
    
    # Context information
    working_directory: Optional[str] = Field(None, description="Current working directory")
    previous_commands: Optional[List[str]] = Field(None, max_items=10, description="Recent commands")
    operating_system: Optional[str] = Field(None, description="Operating system")
    shell_type: Optional[str] = Field(default="bash", description="Shell type (bash, zsh, fish, etc.)")
    user_level: Optional[str] = Field(default="intermediate", description="User experience level")
    ...
    include_explanations: bool = Field(default=True, description="Include command explanations")
...
class CommandSuggestion(BaseModel):
    ...
    safety_level: str = Field(..., description="Safety level (safe, caution, dangerous)")
    
    # Additional details
    examples: Optional[List[str]] = Field(default=[], description="Usage examples")
    alternatives: Optional[List[str]] = Field(default=[], description="Alternative commands")
    prerequisites: Optional[List[str]] = Field(default=[], description="Prerequisites or dependencies")
...
class CommandSuggestionResponse(BaseModel):
    ...
    suggestions: List[CommandSuggestion] = Field(..., description="List of command suggestions")
...
class CommandExplanationRequest(BaseModel):
    ...
    api_key: str = Field(..., min_length=10, description="User's OpenRouter API key")
    command: str = Field(..., min_length=1, max_length=2000, description="Command to explain")
    
    # Context
    working_directory: Optional[str] = Field(None, description="Command context directory")
    user_level: Optional[str] = Field(default="intermediate", description="User experience level")
    include_examples: bool = Field(default=True, description="Include usage examples")
    include_alternatives: bool = Field(default=True, description="Include alternative commands")
...
class CommandExplanation(BaseModel):
    ...
    # Command breakdown
    components: List[Dict[str, str]] = Field(default=[], description="Command components breakdown")
    parameters: List[Dict[str, Any]] = Field(default=[], description="Parameters and flags explanation")
    
    # Additional information
    examples: List[Dict[str, str]] = Field(default=[], description="Usage examples with descriptions")
    alternatives: List[Dict[str, str]] = Field(default=[], description="Alternative commands")
    ...
    best_practices: List[str] = Field(default=[], description="Best practice recommendations")
...
class CommandExplanationResponse(BaseModel):
    ...
    response_time_ms: int = Field(..., description="Response time in milliseconds")
    tokens_used: Dict[str, int] = Field(..., description="Token usage breakdown")
    confidence_score: float = Field(..., ge=0, le=1, description="Explanation confidence score")
...
class ErrorAnalysisRequest(BaseModel):
    ...
    api_key: str = Field(..., min_length=10, description="User's OpenRouter API key")
    command: str = Field(..., min_length=1, max_length=2000, description="Failed command")
    ...
    environment_info: Optional[Dict[str, str]] = Field(None, description="Environment variables")
    system_info: Optional[Dict[str, str]] = Field(None, description="System information")
    
    # Analysis preferences
    include_solutions: bool = Field(default=True, description="Include solution suggestions")
    include_prevention: bool = Field(default=True, description="Include prevention tips")
...
class ErrorAnalysis(BaseModel):
    ...
    error_category: str = Field(..., description="Error category (permission, not_found, syntax, etc.)")
    root_cause: str = Field(..., description="Identified root cause of the error")
    explanation: str = Field(..., description="Detailed explanation of why the error occurred")
    
    # Solutions
    immediate_fixes: List[Dict[str, str]] = Field(default=[], description="Immediate fix suggestions")
    alternative_approaches: List[Dict[str, str]] = Field(default=[], description="Alternative approaches")
    troubleshooting_steps: List[str] = Field(default=[], description="Step-by-step troubleshooting")
    
    # Prevention
    prevention_tips: List[str] = Field(default=[], description="Prevention recommendations")
    best_practices: List[str] = Field(default=[], description="Best practices to avoid similar errors")
    
    # Additional context
    related_errors: List[str] = Field(default=[], description="Related error patterns")
    resources: List[Dict[str, str]] = Field(default=[], description="Additional resources or documentation")
...
class CommandOptimizationRequest(BaseModel):
    ...
    api_key: str = Field(..., min_length=10, description="User's OpenRouter API key")
    command: str = Field(..., min_length=1, max_length=2000, description="Command to optimize")
    
    # Context
    usage_frequency: Optional[str] = Field(None, description="How often the command is used")
    performance_issues: Optional[str] = Field(None, description="Specific performance concerns")
    ...
    optimize_for: str = Field(default="performance", description="Optimization target (performance, safety, readability)")
    include_modern_alternatives: bool = Field(default=True, description="Include modern tool alternatives")
...
class CommandOptimization(BaseModel):
    ...
    optimized_commands: List[Dict[str, Any]] = Field(..., description="Optimized alternatives")
    
    # Analysis
    performance_analysis: Dict[str, Any] = Field(..., description="Performance analysis")
    bottlenecks_identified: List[str] = Field(default=[], description="Identified bottlenecks")
    improvements_made: List[str] = Field(default=[], description="Improvements in optimized versions")
    
    # Recommendations
    best_practices: List[str] = Field(default=[], description="Best practice recommendations")
    modern_alternatives: List[Dict[str, str]] = Field(default=[], description="Modern tool alternatives")
    
    # Trade-offs
    trade_offs: List[Dict[str, str]] = Field(default=[], description="Trade-offs to consider")
    compatibility_notes: List[str] = Field(default=[], description="Compatibility considerations")
...
class CommandOptimizationResponse(BaseModel):
    ...
    response_time_ms: int = Field(..., description="Response time in milliseconds")
    tokens_used: Dict[str, int] = Field(..., description="Token usage breakdown")
    confidence_score: float = Field(..., ge=0, le=1, description="Optimization confidence score")
...
class AISettings(BaseModel):
    ...
    preferred_models: Dict[str, str] = Field(default={}, description="Preferred models for different tasks")
    default_model: Optional[str] = Field(None, description="Default model")
    
    # Response preferences
    max_suggestions: int = Field(default=5, ge=1, le=10, description="Maximum suggestions")
    detail_level: str = Field(default="medium", description="Default detail level")
    include_examples: bool = Field(default=True, description="Include examples by default")
...
class AIModelInfo(BaseModel):
    ...
    architecture: Dict[str, Any] = Field(default={}, description="Architecture information")
    
    # Usage recommendations
    recommended_for: List[str] = Field(default=[], description="Recommended use cases")
    performance_tier: str = Field(..., description="Performance tier (fast, balanced, powerful)")
...
class AvailableModelsResponse(BaseModel):
    ...
    recommended_models: List[str] = Field(..., description="Recommended models for DevPocket")
    timestamp: datetime = Field(..., description="Response timestamp")
