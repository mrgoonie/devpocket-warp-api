...
from .schemas import (
    # API Key schemas
    APIKeyValidation, APIKeyValidationResponse, AIUsageStats,
    
    # Command AI schemas
    CommandSuggestionRequest, CommandSuggestionResponse,
    CommandExplanationRequest, CommandExplanationResponse,
    ErrorAnalysisRequest, ErrorAnalysisResponse,
    CommandOptimizationRequest, CommandOptimizationResponse,
    
    # Settings and models
    AISettings, AISettingsResponse, AvailableModelsResponse,
    
    # Batch processing
    BatchAIRequest, BatchAIResponse,
    
    # Common schemas
    MessageResponse, AIErrorResponse
)
...
async def validate_api_key(
    ...
    if result.valid:
        ...
        logger.warning(f"API key validation failed for user {current_user.username}: {result.error}")
...
async def optimize_command(
    ...
    try:
        ...
        logger.error(f"Command optimization error for user {current_user.username}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def process_batch_requests(
    ...
    try:
        ...
        logger.info(f"Batch AI processing completed for user {current_user.username}: "
