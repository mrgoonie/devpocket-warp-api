...
from .schemas import (
    ...
    SessionCommand, SessionCommandResponse,
    SessionSearchRequest, SessionStats,
    SessionHistoryResponse, SessionHistoryEntry,
    WSMessage, SessionHealthCheck
)
...
class SessionService:
    ...
    async def create_session(
        ...
        try:
            ...
            if session_data.ssh_profile_id:
                ssh_profile = await self.ssh_profile_repo.get_by_id(session_data.ssh_profile_id)
            ...
            if existing_session and existing_session.status in ['active', 'connecting']:
                raise HTTPException(
            ...
            logger.info(f"Terminal session created: {created_session.name} by user {user.username}")
    ...
    async def get_user_sessions(
        ...
        try:
            ...
            # Get total count
            total = await self.session_repo.count_user_sessions(user.id, active_only=active_only)
            ...
            for session_obj in sessions:
                ...
                if session_obj.id in self._active_sessions:
                    ...
                    session_obj.last_activity = memory_session.get("last_activity", session_obj.last_activity)
    ...
    async def get_session(self, user: User, session_id: str) -> SessionResponse:
        ...
        if session_id in self._active_sessions:
            ...
            session_obj.status = memory_session.get("status", session_obj.status)
            session_obj.last_activity = memory_session.get("last_activity", session_obj.last_activity)
            session_obj.command_count = memory_session.get("command_count", session_obj.command_count)
    ...
    async def update_session(
        ...
        try:
            ...
            if "terminal_size" in update_dict:
                ...
                if terminal_size:
                    update_dict["terminal_cols"] = terminal_size.get("cols", session_obj.terminal_cols)
                    update_dict["terminal_rows"] = terminal_size.get("rows", session_obj.terminal_rows)
            ...
            logger.info(f"Terminal session updated: {session_obj.name} by user {user.username}")
    ...
    async def terminate_session(self, user: User, session_id: str, force: bool = False) -> bool:
        ...
        try:
            ...
            logger.info(f"Terminal session terminated: {session_obj.name} by user {user.username}")
            return True
    ...
    async def delete_session(self, user: User, session_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Terminal session deleted: {session_obj.name} by user {user.username}")
            return True
    ...
    async def get_session_stats(self, user: User) -> SessionStats:
        ...
        try:
            ...
            sessions_today = len([
                s for s in stats_data["sessions"]
    ...
    async def check_session_health(self, user: User, session_id: str) -> SessionHealthCheck:
        ...
        try:
            ...
            uptime = 0
            if session_obj.start_time:
                uptime = int((datetime.utcnow() - session_obj.start_time).total_seconds())
