...
from .schemas import (
    ...
    SessionSearchRequest, SessionStats, SessionHealthCheck,
    ...
    # Batch operations
    BatchSessionOperation, BatchSessionResponse,
...
async def get_session_history(
    ...
    service = SessionService(db)
    return await service.get_session_history(current_user, session_id, limit=limit, offset=offset)
    ...
    for session_id in operation.session_ids:
        try:
            if operation.operation == "terminate":
                await service.terminate_session(current_user, session_id, force=operation.force)
                results.append({
                    "session_id": session_id,
                    "status": "success",
                    "operation": "terminate"
...
async def cleanup_inactive_sessions(
    current_user: Annotated[User, Depends(get_current_active_user)],
    db: Annotated[AsyncSession, Depends(get_db)],
    max_idle_hours: int = Query(default=24, ge=1, le=168, description="Max idle time in hours")
) -> dict:
    ...
    for session in all_sessions:
        try:
            ...
            if (session.status in ["active", "connecting"] and 
...
async def get_session_metrics(
    ...
    try:
        ...
        return {
            ...
            "averages": {
                "session_duration_minutes": round(stats.average_session_duration_minutes, 2),
