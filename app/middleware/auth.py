...
class AuthenticationMiddleware(BaseHTTPMiddleware):
        ...
        if token:
            try:
                ...
                if payload:
                    ...
                    logger.debug(f"User authenticated via middleware: {payload.get('sub')}")
                else:
    ...
    def _log_request(self, request: Request, response: Response, process_time: float) -> None:
        ...
        if (getattr(request.state, 'is_authenticated', False) or 
            ...
            if getattr(request.state, 'user_id', None):
                ...
                log_data["subscription_tier"] = getattr(request.state, 'subscription_tier', 'unknown')
