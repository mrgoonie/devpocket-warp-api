...
class OpenRouterService:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        if auth_response.status_code == 200:
                            ...
                            account_info = {
                                "label": account_data.get("data", {}).get("label", "Unknown"),
                                "usage": account_data.get("data", {}).get("usage", 0),
                                "limit": account_data.get("data", {}).get("limit"),
                                "is_free_tier": account_data.get("data", {}).get("is_free_tier", True)
                            }
                    except Exception:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    raise Exception(f"Failed to fetch usage stats: {response.status_code}")
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    raise Exception(f"OpenRouter API error: {response.status_code} - {error_data}")
                    
        except httpx.TimeoutException:
            raise Exception("Request timeout - OpenRouter API is taking too long to respond")
        except Exception as e:
    ...
    async def _check_rate_limit(self, api_key: str) -> bool:
        ...
        # Clean old requests
        self._rate_limits[api_key] = [
            req_time for req_time in self._rate_limits[api_key]
