...
class Command(BaseModel):
    ...
    def complete_execution(self, exit_code: int, output: str = None, error_output: str = None) -> None:
    ...
    def classify_command(self) -> str:
        ...
        # File operations
        elif any(command_lower.startswith(cmd) for cmd in ['ls', 'cd', 'mkdir', 'rmdir', 'rm ', 'cp ', 'mv ', 'find', 'locate']):
            return "file_operation"
        
        # Network commands
        elif any(command_lower.startswith(cmd) for cmd in ['ping', 'curl', 'wget', 'ssh', 'scp', 'rsync', 'netstat']):
            return "network"
        
        # System commands
        elif any(command_lower.startswith(cmd) for cmd in ['ps', 'top', 'htop', 'kill', 'systemctl', 'service', 'df', 'du', 'mount', 'umount']):
            return "system"
        
        # Package management
        elif any(command_lower.startswith(cmd) for cmd in ['apt', 'yum', 'dnf', 'pip', 'npm', 'yarn', 'brew']):
            return "package_management"
        
        # Development
        elif any(command_lower.startswith(cmd) for cmd in ['docker', 'kubectl', 'make', 'cmake', 'gcc', 'python', 'node', 'java']):
            return "development"
    ...
    def check_sensitive_content(self) -> bool:
        ...
        sensitive_patterns = [
            ...
            'credential', 'api_key', 'private', 'ssh-keygen'
        ]
...
Index('idx_commands_user_command', Command.session_id, Command.command)  # For command history by user
Index('idx_commands_ai_suggested', Command.was_ai_suggested, Command.created_at)  # For AI analytics