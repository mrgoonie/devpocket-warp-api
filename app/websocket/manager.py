...
from .protocols import (
    TerminalMessage, MessageType, parse_message,
    create_error_message, create_status_message, HeartbeatMessage
)
...
class ConnectionManager:
    ...
    async def handle_message(
        ...
        try:
            ...
            elif message.type in [MessageType.INPUT, MessageType.RESIZE, MessageType.SIGNAL]:
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                session_data = {
                    ...
                    "terminal_cols": message.data.get("terminal_size", {}).get("cols", 80),
                    "terminal_rows": message.data.get("terminal_size", {}).get("rows", 24),
                    "is_active": True
                }
    ...
    async def _handle_terminal_message(
        ...
        if not message.session_id:
            error_msg = create_error_message("missing_session_id", "Session ID required")
            await connection.send_message(error_msg)
            return
        ...
        if message.type == MessageType.INPUT:
            ...
            await session.handle_resize(message.data.get("cols", 80), message.data.get("rows", 24))
