...
from .protocols import create_output_message, create_status_message, create_error_message
    ...
    async def stop(self) -> None:
        ...
        if self.db_session and self.db:
            try:
                ...
                await session_repo.update(self.db_session.id, {
                    "is_active": False,
    ...
    async def handle_input(self, data: str) -> None:
        ...
        try:
            ...
            if self.ssh_handler:
                ...
                logger.warning(f"No handler available for input in session {self.session_id}")
                return
            
            if not success:
                await self._send_error("input_failed", "Failed to send input to terminal")
    ...
    async def handle_resize(self, cols: int, rows: int) -> None:
        ...
        try:
            ...
            if self.db_session and self.db:
                try:
                    session_repo = SessionRepository(self.db)
                    await session_repo.update(self.session_id, {
                        "terminal_cols": cols,
                        "terminal_rows": rows
                    })
                    await self.db.commit()
            ...
            if success:
                logger.debug(f"Terminal resized to {cols}x{rows} for session {self.session_id}")
            else:
    ...
    async def handle_signal(self, signal_name: str) -> None:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"Failed to send signal {signal_name} to session {self.session_id}")
