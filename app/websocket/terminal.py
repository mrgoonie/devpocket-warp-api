...
class TerminalSession:
        ...
        if self.db_session and self.db:
            try:
                ...
                await session_repo.update(
                    self.db_session.id, {"is_active": False, "ended_at": datetime.now()}
                )
        ...
        try:
            ...
            if self.db_session and self.db:
                try:
                    session_repo = SessionRepository(self.db)
                    await session_repo.update(
                        self.session_id, {"terminal_cols": cols, "terminal_rows": rows}
                    )
                    await self.db.commit()
                except Exception as e:
    ...
    async def _start_pty_session(self) -> bool:
        ...
        try:
            ...
            self.pty_handler = PTYHandler(
                output_callback=self._handle_output, rows=self.rows, cols=self.cols
            )
    ...
    async def _send_status(
        self, status: str, message: str = "", server_info: Optional[dict] = None
    ) -> None:
