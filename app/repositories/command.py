...
class CommandRepository(BaseRepository[Command]):
    ...
    async def get_commands_by_status(
        ...
        if user_id:
            from app.models.session import Session
            query = (
                query.join(Session, Command.session_id == Session.id)
                .where(Session.user_id == user_id)
            )
    ...
    async def get_command_stats(self, user_id: str = None) -> dict:
        ...
        if user_id:
            base_query = (
                base_query.join(Session, Command.session_id == Session.id)
                .where(Session.user_id == user_id)
            )
    ...
    async def get_top_commands(
        ...
        query = (
            select(Command.command, func.count(Command.id).label('usage_count'))
