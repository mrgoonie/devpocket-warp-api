...
class SyncDataRepository(BaseRepository[SyncData]):
    ...
    async def get_user_sync_data(
        ...
        query = query.order_by(desc(SyncData.last_modified_at)).offset(offset).limit(limit)
    ...
    async def create_or_update_sync_item(
        ...
        if existing_item:
            # Check for conflicts
            if (existing_item.last_modified_at > datetime.now() - timedelta(seconds=1) and
                existing_item.source_device_id != device_id and
    ...
    async def get_sync_changes_since(
        ...
        query = select(SyncData).where(
            and_(
                SyncData.user_id == user_id,
                SyncData.last_modified_at > since
            )
        )
